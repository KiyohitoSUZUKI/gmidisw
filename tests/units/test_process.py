...
import midisw.process as test_target
...
class TestProcess(unittest.TestCase):
    ...
    def test_start_to_stop(self):
        ...
        for profname in self.target_profile_names:
            ...
            loaded_profs[profname] = test_profile.load('process',profname)
        ...
        for profname in self.target_profile_names:
            ...
            for port in p_wrapper[profname].prof['provides']:
                self.assertTrue(midisw.util.is_port_active(port['type'],port['name']))
        ...
        for profname in reversed(self.target_profile_names):
            ...
            for port in p_wrapper[profname].prof['provides']:
                self.assertFalse(midisw.util.is_port_active(port['type'],port['name']))
